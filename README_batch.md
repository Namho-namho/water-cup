...
`params.csv` 에 인덱스 1~1000의 유형·수위·파라미터가 들어 있고, 배치 잡이 이 파일에서
...
물을 넘치게 하는 건 가속도 자체보다 공진입니다. 안반경 28mm 원통의 1차 슬로싱은
4.04Hz이고 이동 구간이 0.8초이므로 **0.8초에 3.23주기**가 공진입니다(수위 55~85mm
모두 같습니다). 그래서 shake/zigzag의 주기 수를 공진 근처(2.2~3.6)에 두고, 진폭은
가속도 예산에서 역산합니다.

    흔들림 가속도 a ≈ K · 진폭 · 주기수²      (zigzag K=27, shake K=30, curve K=55)

`ACC_MAX`(기본 5.0 m/s²)와 `ACC_MIN`(기본 2.0)으로 예산을 조절하고, 예상값은
params.csv 의 `acc_pred` 열에 기록됩니다. 1000개 분포는 0.8~5.1 m/s²(중앙 2.6)입니다.

`step` 은 조각 길이만 바꿀 뿐 이동 구간 길이(200스텝 = 100프레임 = 0.8초)를 바꾸지
않으므로 가속도에 영향이 없습니다. 가속도는 진폭과 주기 수로만 조절됩니다.

넘침 기준은 수위에 따라 갈립니다. 벽에서의 수면 상승은 대략 28mm·a/g 이므로,
85mm 수위는 a ≳ 3.2 m/s² 부터 실제로 넘치고, 55~65mm 는 이 예산 안에서는 넘치지
않고 출렁이기만 합니다.
