...
//! clamp particles into a cylindrical cup interior (for moving containers)
PYTHON() void clampToCup(BasicParticleSystem& parts, ParticleDataImpl<Vec3>& pvel, Vec3 center, Real innerR, Real outerR, Real floorY, Real rimY, Real eps=0.15) {
	for (IndexInt idx=0; idx<parts.size(); idx++) {
		if (!parts.isActive(idx)) continue;
		Vec3 p = parts.getPos(idx);
		bool ch = false;
		if (p.y < rimY) {
			Real dx = p.x - center.x, dz = p.z - center.z;
			Real r = std::sqrt(dx*dx + dz*dz);
			Real rmax = innerR - eps;
			if (r > rmax && r < outerR && r > 1e-6) {
				Real sc = rmax / r; p.x = center.x + dx*sc; p.z = center.z + dz*sc; ch = true;
				Vec3 n(dx/r, 0.0, dz/r);
				Real vn = dot(pvel[idx], n);
				if (vn > 0) pvel[idx] -= n * vn;
			}
			if (r < outerR && p.y < floorY + eps) { p.y = floorY + eps; ch = true; if (pvel[idx].y < 0) pvel[idx].y = 0; }
		}
...
PYTHON() void clampToCupAxis(BasicParticleSystem& parts, ParticleDataImpl<Vec3>& pvel,
	...
	for (IndexInt idx=0; idx<parts.size(); idx++) {
		...
		Vec3 d = p - base;
		Real h = dot(d, a);
		if (h > height) continue;          // above the rim: free (spilling allowed)
		Vec3 rad = d - a*h;
		Real r = norm(rad);
		bool ch = false;
		Real rmax = innerR - eps;
		...
		if (r < outerR && h < eps) {
			p = p + a*(eps - h); ch = true;
			Real vn = dot(pvel[idx], a);
			if (vn < 0) pvel[idx] -= a * vn;
		}
		if (ch) parts.setPos(idx, p);
	}
}
