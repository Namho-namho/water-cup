...
Blender FLIP Fluids는 같은 시나리오에서 부피가 -13.9% 사라지지만, IDP 적용판은 1% 이내입니다.
...
컵 안지름 56mm / 높이 100mm / 바닥 6mm, 궤적 225프레임(1.79초) 기준.
"설정 수위"와 "실제 수위"가 다른 이유는 위 주의사항 참고.

**현재 버전** (초기 물기둥을 컵 안벽까지 채움)

| 궤적 | 회전 | 설정 수위 | 실제 수위 | 벽 넘어감(넘침) | 비고 |
|---|---|---|---|---|---|
| traj_slow225 | 1.6도 | 85mm | 92.5mm | (측정 예정) | 배포 기본 설정 |
| traj_tilt | 8.1도 | 85mm | 92.5mm | 5.61% (f82) | 기동 중반에 넘침 |
| traj_tilt | 8.1도 | 92mm | 97.3mm | 5.84% (f7) | 정착 직후부터 넘침 |

※ 넘침이 발생하면 흘러나온 물이 도메인 바닥에 퍼지면서 메시 부피가 과대평가됩니다
   (traj_tilt 85mm 케이스에서 +27.7%). 부피 보존 지표는 넘치지 않는 조건에서만 의미가 있습니다.

**이전 버전** (물기둥을 컵 안반경보다 2mm 좁게 만들어 실제 수위가 설정값보다 낮았음)

| 궤적 | 회전 | 설정 수위 | 부피 변화 | 넘침 | 기울기 평균 / RMS |
|---|---|---|---|---|---|
| traj_slow225 | 1.6도 | 55mm | — | 0% | 9.30 / 10.86 mm |
| traj_tilt | 8.1도 | 55mm | -0.45% | 0% | 12.13 / 14.47 mm |
| traj_tilt | 8.1도 | 85mm | -0.31% | 0% | — |
| traj_tilt | 8.1도 | 92mm | -0.09% | 0.02% | — |

공통: 누수 0 / 표면 요철 RMS 0.068mm / 라벨 32x32 유효 616셀
기울기는 컵 좌표계에서 수면에 평면을 맞춰 구한, 지름 54mm에 걸친 높이차입니다.

## 미해결

- 실기 검증 미완료: 시뮬레이터마다 슬로싱 크기가 크게 다릅니다. 실제 로봇으로 같은 궤적을
  실행해 수면 기울기를 계측해야 어느 쪽이 맞는지 확정됩니다.
- 격자 152x152x208 고정. 더 먼 거리를 이동하는 궤적은 격자를 키워야 합니다.
- 컵 치수는 CONFIG로 바꿀 수 있지만 렌더용 blend의 컵 오브젝트는 따로 맞춰야 합니다.
- 수위 자동 보정 미구현: 위 '실제 수위' 항목 참고. 정착-측정-재생성 반복 구조를 넣으면
  자동화 가능하지만 시뮬 시간이 늘어납니다.
